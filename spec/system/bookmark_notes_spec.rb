...
RSpec.describe "Bookmark personal notes" do
  fab!(:current_user) do
    Fabricate(:user, refresh_auto_groups: true, uploaded_avatar: Fabricate(:image_upload))
  end
  fab!(:other_user) do
    Fabricate(:user, refresh_auto_groups: true, uploaded_avatar: Fabricate(:image_upload))
  end
  fab!(:topic)
  fab!(:post) do
    Fabricate(:post, topic:, user: current_user, raw: "Research material for the team")
  end
  ...
  it "creates, updates, and removes a Markdown note without losing the bookmark reminder" do
    ...
    expect(bookmark_modal).to have_note("Follow up with **Sarah** about the results.")
    expect(bookmark_modal).to have_existing_reminder
  ...
  it "edits an existing personal note from the bookmarks list" do
    bookmark = Fabricate(:bookmark, user: current_user, bookmarkable: post, name: "Reading notes")
  ...
  it "keeps Enter inside the rich text note editor" do
    ...
    expect(bookmark_modal).to be_open
    expect(bookmark_modal).to have_rich_note_paragraphs("First line", "Second line")
  ...
  it "shows each user only their own note on the same post" do
    current_user_bookmark = Fabricate(:bookmark, user: current_user, bookmarkable: post)
    other_user_bookmark = Fabricate(:bookmark, user: other_user, bookmarkable: post)
    Fabricate(:bookmark_note, bookmark: current_user_bookmark, raw: "Current user's private note")
    Fabricate(:bookmark_note, bookmark: other_user_bookmark, raw: "Other user's private note")
